...
# VIX Gate: Skip trading when INDIA VIX is below this threshold.
# Data proves intraday index ranges are too narrow to sustain trend-following
# when VIX < 15 (avg range 0.94% vs 1.24%+ when VIX > 15).
...
def fetch_india_vix() -> float | None:
    """Fetch live INDIA VIX value from the API.
    Returns the VIX float or None if unavailable."""
    try:
...
def run_strategy():
    ...
    while True:
        ...
        # VIX Gate: Check INDIA VIX before allowing any entries today
        vix_val = fetch_india_vix()
        if vix_val is not None and vix_val < VIX_GATE_MIN:
            log.info(
                f"⛔ VIX GATE ACTIVE: INDIA VIX = {vix_val:.2f} < {VIX_GATE_MIN:.1f} threshold. "
                f"Intraday range too narrow for trend-following. Skipping today."
            )
            ...
            log.info(f"✅ VIX GATE PASSED: INDIA VIX = {vix_val:.2f} >= {VIX_GATE_MIN:.1f}. Proceeding with entries.")
        else:
            log.warning("Could not fetch INDIA VIX. Proceeding cautiously (gate bypassed).")
