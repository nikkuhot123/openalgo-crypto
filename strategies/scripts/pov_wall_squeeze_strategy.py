...
# SEBI circular HO/47/11/11(3)2025-MRD-POD2/I/2765/2026 dated 2026-01-16.
# Continuous trading in F&O-underlying CASH stocks ends 15:15; the index spot
# then FREEZES until the auction stamps a price at ~15:28-15:29. Measured on
# NIFTY: +200.95 pts (Aug 3) and +151.45 pts (Aug 4) teleport in a single tick.
#
# This strategy picks its ATM strike from the live underlying spot
# (underlying_ltp). After 15:15 that quote is stale; after ~15:28 it is a
# phantom jump. Either one selects a strike far from the real market. So new
# entries must stop BEFORE the freeze, and open legs must be flat before
# liquidity thins into the auction.
#
# Index OPTIONS themselves keep trading to 15:40 (derivatives segment), so the
# 15:12 flat is comfortably executable.
CAS_NO_NEW_ENTRY_AFTER = os.getenv('CAS_NO_NEW_ENTRY_AFTER', '15:10')  # spot still live
CAS_EOD_FLAT_TIME = os.getenv('CAS_EOD_FLAT_TIME', '15:12')            # force flat by here
...
T_NO_NEW_ENTRY = _hhmm_t(CAS_NO_NEW_ENTRY_AFTER)
T_EOD_FLAT = _hhmm_t(CAS_EOD_FLAT_TIME)
        ...
        try:
            ...
            for option_type, offset in legs:
                ...
                if pos:
                    ...
                    # ── CAS hard EOD flat ──
                    # This strategy has no other time-based exit: its exits are
                    # target / broker-SL / max-hold / decay only. Before CAS the
                    # process simply ran to 15:29 and MIS square-off cleaned up.
                    # That is no longer safe - the cash spot dies at 15:15 and
                    # option liquidity thins into the auction, so a position left
                    # open past the freeze exits at whatever the book offers.
                    # Options trade to 15:40, so a 15:12 market exit fills normally.
                    if datetime.now().time() >= T_EOD_FLAT:
                        log.warning(
                            f"!!! CAS EOD flat ({CAS_EOD_FLAT_TIME}) !!! Closing {symbol} "
                            f"before cash-segment freeze at 15:15..."
                        )
                        outcome, fill_px = verified_exit_sell(
                            symbol, opt_exchange, pos.get("qty", QUANTITY), sl_oid, "CAS-EOD-flat")
                        if outcome == "unknown":
                            continue  # broker unverifiable — retry next cycle
                        if outcome == "sold":
                            consecutive_losses, daily_loss_rs = book_trade_pnl(
                ...
                # CAS: no new entries once the cash spot stops updating (15:15).
                # ATM strike selection reads underlying_ltp, which is stale after
                # the freeze and a phantom jump after the ~15:28 auction stamp.
                if datetime.now().time() >= T_NO_NEW_ENTRY:
                    continue
